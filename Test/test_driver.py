...
class TestChecker:
    ...
    def test_join_df(self):
        ...
        d3_tru = [['a', 401, 32], ['c', 238, 101], ['d', nan, 3]]
        # self.data2_fst = [
        #     ['a', 401],
        #     ['c', 238],
        #     ['d', nan],
        ...
        #     ['d', abs(-3)]    
        # ]
        d4_tru = [['c', 238, 101], ['a', nan, 452], ['d', 1, 3]]
        # self.data2_snd = [
        #     ['e', nan],
        #     ['d', 1],
        #     ['c', 238],
    ...
    def test_sum_df_cols(self):
        ...
        d2_rel = self.checker.sum_df_cols(
        ...
        d3_rel = self.checker.sum_df_cols(
            self.checker.join_df(
        ...
        d4_rel = self.checker.sum_df_cols(
            self.checker.join_df(
                self.__form_df(self.data2_snd, cols=[KEY, COL_1]),
        ...
        d5_rel = self.checker.sum_df_cols(
            self.checker.join_df(
    ...
    def test_avg_df_cols(self):
        ...
        d2_rel = self.checker.avg_df_cols(
            self.checker.join_df(
                self.__form_df(self.data2),
                self.__form_df(self.data3, cols=[KEY, COL_3, COL_4])
            ), 
        ...
        d3_rel = self.checker.avg_df_cols(
            self.checker.join_df(
                self.__form_df(self.data3_fst, cols=[KEY, COL_1]),
                self.__form_df(self.data3_snd, cols=[KEY, COL_2])
            ), 
        ...
        d4_rel = self.checker.avg_df_cols(
            self.checker.join_df(
                self.__form_df(self.data2_snd, cols=[KEY, COL_1]),
                self.__form_df(self.data4, cols=[KEY, COL_2, COL_3])
            ), 
        ...
        d5_rel = self.checker.avg_df_cols(
            self.checker.join_df(
                self.__form_df(self.data3, cols=[KEY, COL_1, COL_2]),
                self.__form_df(self.data4_fst, cols=[KEY, COL_3])
            ), 
        ...
        with pytest.raises(ZeroDivisionError):
            d6_rel = self.checker.avg_df_cols(
                self.__form_df(self.data4),
                1, 
                1,
                COL_AVG
                )